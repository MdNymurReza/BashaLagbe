...
// Component: SeekCard (Individual Seeking Post Card)
// ============================================================

import { useState } from 'react';
import type { SeekingPost } from '../types';
import { fmt, propertyTypeLabel, fmtDate } from '../utils';
import { User, Calendar } from 'lucide-react';
import SeekResponseModal from './SeekResponseModal';
  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
        <div>
          <div style={{ fontSize: '18px', fontWeight: 600, color: 'var(--ink-dark)', marginBottom: '4px' }}>
            {post.user_name || 'Anonymous Student'}
          </div>
        ...
        <div style={{ textAlign: 'right' }}>
          <span className="badge badge-navy">{post.zone || 'UIU Area'}</span>
          ...
          {post.move_in_date && (
            <div style={{ gridColumn: '1/-1' }}>
              <div style={{ fontSize: '12px', color: 'var(--gray)' }}>
                <Calendar size={14} style={{ verticalAlign: 'text-bottom' }}/> Move-in Date
              </div>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid var(--border)' }}>
        ...
        >
          {isLoggedIn ? 'Respond to Post' : 'Log in to Respond'}
        </button>
      </div>
      ...
      {responseModalOpen && (
        <SeekResponseModal 
          ...
          postUser={post.user_name || 'Student'}
        />
      )}
    </div>
  );
